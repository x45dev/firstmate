...
Stop a background process a test started with `fm_test_stop` from `tests/lib.sh`, never with one `kill` followed by a bare `wait`: a bash script that traps the signal can lose it, and the `wait` then blocks silently until CI's job cap cancels the shard.
`fm_test_stop` owns the bound, the repeated signal, and the failure message; its header comment owns the mechanism.

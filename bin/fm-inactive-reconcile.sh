...
report_to_parent() { # <task> <state> <outcome-key> <fingerprint> <pr> <pr-landing>
  local task=$1 state=$2 outcome_key=$3 fingerprint=$4 pr=$5 pr_landing=$6 line
...
report_child_ledger_locked() { # <id> <meta>
  local id=$1 meta=$2 status last previous state note pr pr_landing mode yolo data incarnation fingerprint predecessor_head outcome_key line
...
reconcile_direct_child_locked() { # <id> <meta> <secondmate-id-or-empty> <timeout>
  ...
  [ -n "$RECORD_PENDING" ] || return 0
  if [ -n "$self" ]; then
    if report_to_parent "$id" "$state" "$outcome_key" "$fingerprint" "$pr" "$pr_landing"; then
      mark_reported "$RECORD_PENDING" || return 1
    else
      notice_parent_report_failed "$RECORD_PENDING" "$fingerprint" \

...
# task meta (recorded_prs), each carrying the canonical snapshot's recorded
# pr.landing verdict so a row that is not a confirmed landing path can never be read
# as one, but performs no live GitHub discovery or checks. Live PR discovery/checks
# happen ONLY under --include-prs; all gh coupling lives in that branch and never in
# the canonical snapshot. The default output states explicitly (the prs: line and
# the omitted[] surfaces) what was not requested, so an absence is never ambiguous.
#
...
Default fields: schema, home, generated, prs, in_flight{id,kind,state,repo,name,doing},
  ...
  gates{id,title,blocked_by,reason,owner,filed}, reports{id,path}, recorded_prs{id,url,landing},

...
# no-mistakes publishes the intent verbatim as the PR's Intent section, so it is
# written for a reader of the project's repository without dropping a captain
# requirement.
...
The Definition of done's publication rule also still governs it: the string is published verbatim as the pull request's Intent section, so write it for a reader of the project's repository without dropping a captain requirement.
EOF
}
...
That intent is this brief's one publication exception: no-mistakes publishes it verbatim as the pull request's \`## Intent\` section in the project's own repository, so write it for a reader there, carrying no path outside the project, no fleet role or process noun, and no internal decision history that means nothing in that repository.
Rewriting it for that reader never drops a captain requirement: the pipeline's reviewer treats the intent as authoritative acceptance criteria, so a requirement lost in the rewrite is one the pull request is no longer checked against.

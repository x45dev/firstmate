...
test_local_secondmate_delivers_terminal_ledger_line() {
  ...
  expected="done [key=$key]: child child done: PR https://example.test/owner/repo/pull/1 checks green pr=https://example.test/owner/repo/pull/1 pr_landing=unverified mode=no-mistakes yolo=off"
...
test_secondmate_ledger_delivery_carries_report_and_failure() {
  ...
  grep -Fxq "done [key=$scout_key]: child scout done: report written pr=https://example.test/owner/repo/pull/1 pr_landing=unverified mode=no-mistakes yolo=off report=data/scout/report.md" \
    "$MAIN/state/mate.status" || fail "scout delivery lost its report pointer: $(cat "$MAIN/state/mate.status")"
  grep -Fxq "failed [key=$boom_key]: child boom failed: build broke pr=https://example.test/owner/repo/pull/1 pr_landing=unverified mode=no-mistakes yolo=off" \
    "$MAIN/state/mate.status" || fail "failed line was not delivered under the failed verb: $(cat "$MAIN/state/mate.status")"
  grep -Fxq "done [key=$replaced_key]: child replaced-pr done: PR https://example.test/owner/repo/pull/22 pr=https://example.test/owner/repo/pull/22 pr_landing=unverified mode=no-mistakes yolo=off" \
...
test_pr_field_requires_recorded_pr_or_ready_signal_line() {
  ...
  grep -Fxq "done [key=$ready_key]: child ready done: PR https://example.test/owner/repo/pull/44 checks green pr=https://example.test/owner/repo/pull/44 pr_landing=unverified mode=no-mistakes yolo=off" \
    "$MAIN/state/mate.status" \
...
test_watcher_poll_delivers_child_ledger_line_to_parent() {
  ...
  grep -Fxq "done [key=$key]: child child done: PR https://example.test/owner/repo/pull/1 checks green pr=https://example.test/owner/repo/pull/1 pr_landing=unverified mode=no-mistakes yolo=off" \
    "$MAIN/state/mate.status" \
...
test_reported_pr_carries_its_landing_verdict() {
  ...
  # The verdict belongs to the recorded pr= line; a URL taken from the ready line
  # the parent report delivers is a different pull request that nothing ever checked.
  make_world landing-scraped; bind_secondmate local
  write_landing_child "$MATE" child 'done: PR https://example.test/owner/repo/pull/6 checks green' \
    'pr_landing=confirmed'
  FM_FAKE_CREW_STATE='done' run_reconcile "$MATE" --startup
  grep -Fq 'pr=https://example.test/owner/repo/pull/6 pr_landing=unverified' "$MAIN/state/mate.status" \
    || fail "a scraped pull request inherited an unrelated landing verdict: $(cat "$MAIN/state/mate.status")"

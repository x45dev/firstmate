...
#
# fm_test_stop <pid> [label]
#
# Stop a background process a test started and reap it, within a bound. One
# TERM followed by a bare `wait` is not a stop: a bash script that traps TERM
# can lose that signal outright. GNU bash 5.2 runs a trap that lands while it
...
# cancels the whole shard. The race is per signal, so TERM is repeated every
# FM_TEST_STOP_RETERM_SECONDS; a process still alive at
...
fm_test_stop() {
  local pid=$1 label=${2:-process} ticks=0 reterm bound args stat child
  ...
  while kill -0 "$pid" 2>/dev/null; do
    ...
    if [ "$ticks" -ge "$bound" ]; then
      {
        printf '# %s (pid %s) outlived TERM for %ss; process tree:\n' \
          "$label" "$pid" "$FM_TEST_STOP_BOUND_SECONDS"
      ...
      fail "$label (pid $pid) did not stop within ${FM_TEST_STOP_BOUND_SECONDS}s of TERM: ${args:-command line unavailable}"
    fi
    sleep 0.1
    ticks=$((ticks + 1))

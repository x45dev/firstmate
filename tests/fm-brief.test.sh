...
test_no_mistakes_dod_wording() {
  ...
  # instruction must say where the text lands and who reads it, and must never
  # read as licence to drop a captain requirement while rewriting for that reader.
  ...
  assert_grep "carrying no path outside the project, no fleet role or process noun, and no internal decision history" "$brief" \
    "no-mistakes DOD must name what the published intent may not carry"
  assert_grep "Rewriting it for that reader never drops a captain requirement" "$brief" \
    "no-mistakes DOD must forbid dropping a captain requirement when rewriting the intent"

...
config/watched-contacts.json  optional list of deployed contact endpoints this home proves are really the intake handler, read by the check armed with bin/fm-contact-live-check.sh; LOCAL, gitignored, firstmate-maintained but human-editable, and NOT inherited by secondmate homes; see docs/configuration.md "Contact endpoint checks"
...
state/               runtime records and signals; gitignored
  ...
  <id>.progress-sample  the last movement sample of this task's rendered pane, the verdict it established, and the last time that verdict was `advanced`; bin/fm-progress-lib.sh owns the record, the four verdicts, and the bound past which a recorded advance stops deferring a wedge; it is what bin/fm-watch.sh's own wedge bound and busy-marker check read, while bin/fm-crew-state.sh still reports busy without consulting it; removed by teardown
  ...
  contact-live.check.sh  generated contact endpoint poll shim and its .check-trust binding; present only after bin/fm-contact-live-check.sh arm; its report record .contact-live is keyed by endpoint, which is what keeps one failing endpoint from being reported on every poll and lets a failure that clears be reported as recovered
...
Run `bin/fm-pr-check.sh <id> <PR url>` with the URL copied from that ready signal - it records `pr=`, the forge's `pr_head=` when available, and the landing-target verdict `pr_landing=` in the task's meta, and arms the watcher's merge poll.

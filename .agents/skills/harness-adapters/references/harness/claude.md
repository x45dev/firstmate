...
Primary `.claude/settings.json` registers `../../../bin/fm-turnend-guard.sh --claude` and `../../../bin/fm-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`, and registers the auto-arm alone on `StopFailure` so a turn refused by an API error still re-arms (verified 2026-09-22 on 2.1.278).

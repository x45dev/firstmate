...
| `fm-afk-launch.sh`       | Own away-mode entry (read-back, confirm, record), exit, rollback, any backend terminal lifecycle, and the live-daemon `status` read |
...
| `fm-contact-live-check.sh` | Prove each deployed contact endpoint is really the intake handler, from this machine, and keep the readings that answer apart |

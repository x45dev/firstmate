...
FM_CLAUDE_AUTOARM_SYNC_WAIT_MS=800   # milliseconds of elapsed wall clock the --claude turn-end guard waits for watcher health, the auto-arm's in-progress claim or an open Stop auto-arm generation claim, or a fresh epoch before deciding recovery ownership or failure progression
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates unless measured forward progress, recent validation-run step activity, or task-worktree writes defer it (docs/architecture.md); stale panes whose crew is not provably working surface immediately unless admitted directly to the declared-wait cadence, while a live idle declared wait still surfaces once before that cadence bounds repeats
...
FM_PROGRESS_LATCH_MAX_SECS=240     # longest a measured token-counter advance keeps deferring the wedge escalation after the movement itself stops; matches FM_STALE_ESCALATE_SECS so the deferral covers the quiet window being judged and no longer; zero or invalid values use 240
FM_PROGRESS_MIN_GAP_SECS=5         # shortest gap between two pane samples that can establish stillness; a closer pair answers unknown and keeps the older sample as the anchor; an invalid value uses 5
FM_PROGRESS_MAX_GAP_SECS=1800      # longest gap across which a prior pane sample still counts; an older one is re-taken, and a verdict older than this refuses no busy claim; zero or invalid values use 1800
FM_PROGRESS_FOOTER_LINES=6         # non-blank lines counted from the bottom of the pane as the harness's status-line region for the movement verdict (docs/verification/supervision.md "Wedge evidence"); zero or invalid values use 6
FM_RESURFACE_ANCHOR_TOLERANCE_SECS=5   # seconds a re-surface's anchor may appear to move between two fired rechecks before the watcher's triage log records it as moved; instrumentation only, changes no wake

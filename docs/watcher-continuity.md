...
The same hook is registered on StopFailure, which Claude fires instead of Stop when an API error such as the account limit refuses the turn, and there it arms as a handling successor so the refused turn's still-unacknowledged wake is not re-announced into another refused turn.

...
The first is rendered movement (`bin/fm-progress-lib.sh`), which samples the harness's own status-line region, its token counter, and the rendered lines above them that the previous capture did not hold, and compares two samples rather than reading a state word; a moved token counter or moved content is forward progress, while a status line that merely redraws is proof of life only, because a hung foreground call redraws its clock in exactly the same way.

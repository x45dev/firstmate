...
That deferral re-surfaces on the same `FM_PAUSE_RESURFACE_SECS` cadence as a declared wait, with a reason naming the evidence and saying it is the periodic recheck of deferred progress rather than a doubt about that evidence, and it is bounded to one pruned, depth-bounded, wall-clock-bounded walk (`FM_WORKTREE_WRITE_PRUNE`, `FM_WORKTREE_WRITE_MAXDEPTH`, `FM_WORKTREE_WRITE_TIMEOUT`) taken only in the branch that was about to escalate, never on every poll.
...
Two further sources of positive evidence defer the same escalation on the same cadence and under the same chain marker, so no crew earns a second window by producing a second kind of proof.
The first is rendered movement (`bin/fm-progress-lib.sh`), which samples the harness's own status-line region, its token counter, and the rendered lines above them that the previous capture did not hold, and compares two samples rather than reading a state word; a moved token counter, rendered in both samples, is forward progress, while moved content and a status line that merely redraws are proof of life only, because a hung foreground call redraws its clock and blinks its tool header in exactly the same way.
The second is the validation run's own activity recency on the step it is executing, which is the only signal separating a run waiting on a remote service from a run nothing is executing.
A crew whose authoritative current state is complete is not deferred but suppressed outright, because a finished crew's stillness is what completion looks like and the timer has nothing left to measure; every other terminal state stays idle with something unresolved and keeps escalating.
The same movement evidence answers the opposite error: a harness that marks a turn open and then dies leaves that marker set forever, so a busy verdict over a pane whose counters moved nothing across two samples is not reported as working, and that pane returns to ordinary stale triage rather than being trusted as a live worker.
Where a capture fails or comes back empty, the recorded endpoint is probed first, and only a backend that positively reports it gone retires the window's timers and bookkeeping; an unreadable, ambiguous, or merely quiet endpoint keeps everything it had, so absence has to be proven rather than inferred.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review; ahead of every other source it consults `bin/fm-allowance-lib.sh`, the single owner of the account-allowance-park verdict, because a worker parked on the account allowance still renders a live pane and a live endpoint and would otherwise be misread as working, and that verdict stops being asserted once the worker's transcript has crossed the reset its own refusal record names ([docs/verification/supervision.md](verification/supervision.md#allowance-park-detection) owns the per-harness signal evidence, the bound, and the resume `bin/fm-allowance-resume-lib.sh` performs from the watcher). Once that reports not parked, it attributes an active or terminal no-mistakes run under the shared run-attribution contract, then keeps that run-step authoritative even if the pane has closed, except that a `blocked:` event reporting a refused or missing daemon socket outranks a potentially stale active run record.
...
Claude's `bin/fm-claude-stop-autoarm.sh` hook fires on every Stop, and on StopFailure so an API-refused turn still re-arms, and, when the home is eligible and still needs supervision, claims one home-scoped cycle, foregrounds the arm wrapper, and translates actionable closes into exit-2 rewakes; [`watcher-continuity.md`](watcher-continuity.md) owns the StopFailure handling-successor arm that keeps a refused turn from re-announcing its own wake.
...
A presence-gated sub-supervisor (`bin/fm-supervise-daemon.sh`) still extends this for walk-away supervision on the other harnesses: the `/afk` skill starts it through `bin/fm-afk-launch.sh start`, which runs the foreground helper `bin/fm-afk-start.sh` in a non-visible terminal the harness does not own, once the record exists; `bin/fm-afk-launch.sh status` answers whether a live daemon is actually supervising, never the `state/.afk` flag alone, after which the watcher reverts to daemon-managed one-shot mode and the daemon self-handles routine wakes in bash.

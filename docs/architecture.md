...
A completion bounds the same repetition on its own, without any backlog hold: where the last line reports the work done, its first sight alarms and later pane hashes carrying that same unchanged line are absorbed for one `FM_PAUSE_RESURFACE_SECS` window, because a finished crew's idle pane is the outcome rather than a symptom.
Only the completion verb takes that bound: a failure, a blocker, and an open decision each leave something unresolved, so they keep alarming on every new hash exactly as they did before it.
...
The first is rendered movement (`bin/fm-progress-lib.sh`), which samples the harness's own status-line region, its token counter, and the rendered content above them, and compares two samples rather than reading a state word; a moved token counter or moved content is forward progress, while a status line that merely redraws is proof of life only, because a hung foreground call redraws its clock in exactly the same way.

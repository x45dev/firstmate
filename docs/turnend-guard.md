...
It records the publishing pid and its `fm_pid_identity`, and the guard accepts it only while that pid is alive and its identity still matches, the same standard it applies to an open generation claim, so a dead publisher or a reused pid proves nothing.
